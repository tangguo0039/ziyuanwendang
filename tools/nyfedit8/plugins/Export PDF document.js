...
//sHint=Export text contents as a single .pdf file
...
//try{
	...
	if(xNyf.isOpen()){
		...
		let sCfgKeyRange='ExportPdfDoc.iRange', sCfgKeyDstFn='ExportPdfDoc.sDstFn', sCfgKeyImgStg='ExportPdfDoc.iImgStg', sCfgKeyPageSize='ExportPdfDoc.iPageSize';
		...
		let xInitFn=new CLocalFile(localStorage.getItem(sCfgKeyDstFn)||platform.getHomePath()||'', _validate_filename(sTitle||'untitled')); xInitFn.changeExtension('.pdf');
		...
		let vFields = [
			    {sField: "savefile", sLabel: _lc2('DstFn', 'Save as .pdf file'), sTitle: plugin.getScriptTitle(), sFilter: 'PDF documents (*.pdf);;All files (*.*)', sInit: xInitFn.toStr(), bReq: true}
			    ...
			    , {sField: "combolist", sLabel: _lc2('PageSize', 'Page size'), vItems: vPageSize, sInit: localStorage.getItem(sCfgKeyPageSize)||'0', bReq: true}
		    ];

		let vRes = input(plugin.getScriptTitle(), vFields, {nMinSize: 500, vMargins: [2, 0, 50, 0], bVert: true});
		if(vRes && vRes.length>=3){

			let j=0, sDstFn=vRes[j++], iRange=vRes[j++], iImgStg=parseInt(vRes[j++]), iPageSize=parseInt(vRes[j++]);
			if(sDstFn){
				...
				localStorage.setItem(sCfgKeyPageSize, iPageSize);
				...
				if(s){
					...
					var c_sInfoFooter='Generated with <a href="http://www.wjjsoft.com/mybase.html?ref=pdf_export" target=_blank>' + plugin.getAppTitle() + ' ' + plugin.getAppVerStr() + '</a>';
					...
					let ba=platform.convertHtmlToPdf(sHtml, {bAutoImages: true, bPrintBackgrounds: true, sPageSize: vPageSize[iPageSize], nMsecTimeout: 5000});
					if(ba && ba.size()>0){
						if(ba.saveToFile(xDstFn.toStr())>0){
							var sMsg=_lc2('Done', 'Successfully exported text contents and saved as the .pdf document. View it now?')+'\n\n'+xDstFn.toStr();
							if(confirm(sMsg)){
								xDstFn.launch('');
							}
						}
					}else{
						alert('Failure exporting .pdf document.');
					}
				}else{
