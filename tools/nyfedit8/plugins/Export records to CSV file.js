...
//sHint=Export data records to CSV file
...
//try{
	...
	if(xNyf.isOpen()){
		...
		//var xInitFn=new CLocalFile(localStorage.getItem(sCfgKey2)||platform.getHomePath()||'', _validate_filename(sItemTitle)||'untitled' + '.csv');
		var xInitFn=new CLocalFile(localStorage.getItem(sCfgKey2)||platform.getHomePath()||'', _validate_filename(xNyf.getFolderHint(plugin.getCurInfoItem(-1)))||'untitled');
		...
		var vInputFields = [
			...
			, {sField: 'savefile', sLabel: _lc2('File', 'Save as .csv file'), sTitle: plugin.getScriptTitle(), sFilter: 'Comma-Separated Values (*.csv);;All files (*.*)', sInit: xInitFn.toStr()}
				...
				if(sCurItem){
					...
					if(vRecords && vRecords.length>0){
						...
						if(sCsv){
							...
							if(nBytes>0){
								if(confirm(_lc2('Done', 'Successfully exported data records to the CSV file. View it now?')+'\n\n'+sFnDst)){
									//xFnDst.exec('');
									textbox({
								...
								alert(_lc2('Failure', 'Failed to export data records to CSV file.')+'\n\n'+sFnDst);
							}
						}
					}
				}
