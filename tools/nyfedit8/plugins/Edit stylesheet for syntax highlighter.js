...
//sHint=Edit stylesheet for syntax highlighter for Rich/Plain/Markdown text editor
//sCategory=MainMenu.Tools; SyntaxHighlight.Hlqt
//sCondition=
//sID=p.CssForHlqt
//sAppVerMin=8.0
//sAuthor=wjjsoft
...
//try{
	...
	if(1){
		let sFilter='hlqt*.css', sAttr='files+readable+writable', sSort='name';
	...
	let sCfgKey1='CssForHlqt.iSel';

	while(1){

		var vFields = [
			{sField: "combolist", sLabel: _lc2('Files', 'List of stylesheet files customizable for syntax highlighting under current UI theme:'), vItems: vCssFiles, nInit: localStorage.getItem(sCfgKey1)||0}
		];
		...
		if(vRes && vRes.length>0){
			...
			if(iSel>=0 && iSel<vCssFiles.length){
				...
				if(xFn.exists()){
					...
					var sMsg=_lc2('Stylesheet', 'Stylesheet for syntax highlighting') + ": " + xFn.toStr();
					sCss=textbox({
