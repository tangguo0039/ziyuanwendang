	...
	if(xNyf.isOpen()){
		...
		if(!xNyf.isReadonly()){
			...
			if(plugin.isContentEditable()){
					...
					for(var i in v){
						...
						if(typeof(s)=='object' && s.sKey){
							sHtml+='<div>'
								...
								+ platform.htmlEncode(s.sKey||'')
								+ '</span>'
								+ '='
								+ '<span style="padding-left: 1em;">'
								+ platform.htmlEncode(s.sVal||'')
								+ '</span>'
								+ '</div>'
								;
						}else{ //if(typeof(s)=='string')
							if(s){
								sHtml+='<div>' + platform.htmlEncode(s||'') + '</div>';
							}else{
								sHtml+='<div>' + '<br />' + '</div>';
							}
						}
				...
				var sTxt=_trim(plugin.getSelectedText(-1, false)||'');

				//var bFullContent=false;
				//if(!sTxt){
				...
				if(sTxt){
					...
					if(vFields.length>=0){
						...
						var nTotalFields=v0.length+1;
						if(vRes && vRes.length>=nTotalFields){
