...
//try{
	...
	if(xNyf.isOpen()){
		...
		var vScope=[
			...
			, (localStorage.getItem(sCfgKey4)||'true')+'|'+_lc2('TextContents', 'Text contents')
		...
		var vFields = [
			...
			, {sField: "combolist", sLabel: _lc2('OutlineScope', 'Outline scope:'), vItems: vRange, nInit: localStorage.getItem(sCfgKey2)||'', bReq: false}
				...
				for(var iDbPos in mToScan){
					...
					if(vII.length>0){
						...
						for(var i in vII){
							...
							var bDefNote=plugin.isReservedNoteFn(sSsgName);

							var nSize=xDb.getFileSize(xSsgFn.toStr(), false);
							...
							if(sSsgName){
								...
								var sHint=xDb.getFileHint(xSsgFn.toStr());
								if(sHint) sTxt+='\n'+sHint;
							}else{
